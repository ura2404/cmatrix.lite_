...
class Session {
    ...
    private function check(){
        if(cm\App::instance()->Sapi === 'CLI'){
            ...
            $CookieName = str_replace('.','_',cm\Hash::getFile(CM_TOP.'/config.json')->getValue('project/code'));
            $this->Hid = empty($_COOKIE[$CookieName]) ? null : $_COOKIE[$CookieName];
            ...
            if(!$this->Hid){
                ...
                $this->setCookie($CookieName,$this->Hid);
            }
            // какая-то старая сессия
                ...
                if($Session->IsEmpty){
                    ...
                    $this->unsetCookie($CookieName,$this->Hid);
                }
                else{
    ...
    // --- --- --- --- ---
    private function setCookie($cookieName,$hid){
        //$Path = cm\Hash::getFile(CM_TOP.'/config.json')->getValue('www/root');
        $Path = '/';
        
        // --- выставить куку на год
        $Days = 1;    // кол-во дней для куки
        setcookie($cookieName,$hid,time() + ($Days * 86400),$Path);
        setcookie($cookieName.'_ts',time(),time() + ($Days * 86400),$Path);
    ...
    private function unsetCookie($cookieName){
        ...
        setcookie($cookieName.'_ts','',time()-3600,$Path);
